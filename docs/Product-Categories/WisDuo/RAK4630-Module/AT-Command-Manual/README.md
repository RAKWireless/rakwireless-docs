...
## Content
- [RAK4630 AT Command Manual](#-at-command-manual)
  - [Content](#content)
...
AT Command mode is the default setting of . For direct use of RUI3 API, you need to use API mode to be set via `AT+APM`.
...
If joining fails, make sure your device is within the coverage of the gateway. Also, ensure that the RAK4630 is in LoRaWAN mode via `AT+NWM=1`. The region is correct via `AT+BAND`, and the EUIs and keys are correct.
