...
When no value is returned, the `<value><CR><LF>` output is not returned at all. Every command (except for `ATZ` used for MCU reset) returns a status string, which is
preceded and followed by `<CR><LF>` in a `<CR><LF><Status<CR><LF>` format.
...
* `AT_NO_CLASSB_ENABLE`: End-node has not yet switched to Class B.
...
## Content
- [RAK3172-SiP Command Manual](#rak3172-sip-at-command-manual)
  - [Content](#content)
...
| Command | Input Parameter | Return Value                                        | Return Code |
| ------- | --------------- | --------------------------------------------------- | ----------- |
| `ATZ?`  | -               | `ATZ`: triggers a reset on the module.              | OK          |
...
This command is used to access the battery level in (V) volts. 

| Command    | Input Parameter | Return Value                                 | Return Code |
| ---------- | --------------- | -------------------------------------------- | ----------- |
| `AT+BAT?`  | -               | `AT+BAT`: get the battery level in (V) volts | OK          |
| `AT+BAT=?` | -               | `<float>`                                    | OK          |
...
| `AT+BUILDTIME=?` | -               | *date-time of build*                                | OK <br> AT_BUSY_ERROR |
...
| `AT+REPOINFO=?` | -               | *firmware repo information*                      | OK          |

**Example:**

```
AT+REPOINFO=?
221c02fc,fc5e24fa, fae36344,c79ac2de, f18660d1,07da1fd2,
OK
```
...
This command sets the serial port baud rate. Supported baud rates are 9600, 19200, 57600, and 115200.
...
AT Command mode is the default setting of RAK3172-SiP. For direct use of RUI V3 API, you need to use Binary mode to be set via `AT+APM`.
...
This command provides a way to switch to Binary mode.

| Command   | Input Parameter | Return Value                 | Return Code |
| --------- | --------------- | ---------------------------- | ----------- |
| `AT+APM?` | -               | `AT+APM`: switch to Binary mode | OK          |
| `AT+APM`  | -               | -                            | OK          |
...
| Command      | Input Parameter | Return Value                                              | Return Code |
| ------------ | --------------- | --------------------------------------------------------- | ----------- |
| `AT+NETID?`  | -               | `AT+NETID`: get or set the network ID (NetID) of 3 octets | OK          |
| `AT+NETID=?` | -               | < 3 hex >                                                 | OK          |
...
If joining fails, make sure your device is within the coverage of the gateway. Also, ensure that the RAK3172-SiP is in LoRaWAN mode via `AT+NWM=1`. The region is correct via `AT+BAND`, and the EUIs and keys are correct.
...
| Command          | Input Parameter | Return Value                                                                            | Return Code            |
| ---------------- | --------------- | --------------------------------------------------------------------------------------- | ---------------------- |
| `AT+DCS?`        | -               | `AT+DCS`: get or set the ETSI duty cycle setting. <br> -  0 = disable <br> - 1 = enable | OK                     |
| `AT+DCS?`        | -               | 0 or 1                                                                                  | OK                     |
...
| Command         | Input Parameter | Return Value                                                    | Return Code            |
| --------------- | --------------- | --------------------------------------------------------------- | ---------------------- |
| `AT+DR?`        | -               | `AT+DR`: get or set the data rate (0-7 correspond to DR_0-DR_7) | OK                     |
 ...
 **RU864 / IN865 / EU868 / KR920** and the data rate range of values is 0-5 (DR0-DR5). <br>
...
| Command            | Input Parameter | Return Value                                                                                       | Return Code                               |
| ------------------ | --------------- | -------------------------------------------------------------------------------------------------- | ----------------------------------------- |
| `AT+JN1DL?`        | -               | `AT+JN1DL`: get or set the join accept delay between the end of TX and the join RX window 1 in sec | OK                                        |
...
| Command            | Input Parameter | Return Value                                                                                | Return Code                               |
| ------------------ | --------------- | ------------------------------------------------------------------------------------------- | ----------------------------------------- |
| `AT+JN2DL?`        | -               | `AT+JN2DL`: get the join accept delay between the end of TX and the join RX window 2 in sec | OK                                        |
...
| Command            | Input Parameter | Return Value                                                                      | Return Code                               |
| ------------------ | --------------- | --------------------------------------------------------------------------------- | ----------------------------------------- |
| `AT+RX1DL?`        | -               | `AT+RX1DL`: get or set the delay between the end of TX and the RX window 1 in sec | OK                                        |
| `AT+RX1DL=?`       | -               | `<integer>`                                                                       | OK                                        |
...
| Command      | Input Parameter | Return Value                                                               | Return Code      |
| ------------ | --------------- | -------------------------------------------------------------------------- | ---------------- |
| `AT+RX2DL?`  | -               | `AT+RX2DL`: get the delay between the end of TX and the RX window 2 in sec | OK               |
...
| Command            | Input Parameter                        | Return Value                                                                  | Return Code                               |
| ------------------ | -------------------------------------- | ----------------------------------------------------------------------------- | ----------------------------------------- |
| `AT+RX2DR?`        | -                                      | `AT+RX2DR`: get or set the RX2 window data rate (0-7) correspond to DR_0-DR_7 | OK                                        |
...
| Command      | Input Parameter | Return Value                             | Return Code           |
| ------------ | --------------- | ---------------------------------------- | --------------------- |
| `AT+RX2FQ?`  | -               | `AT+RX2FQ`: get the Rx2 window frequency | OK                    |
...
**EU868 / RU864 / KR920 / AS923** and Transmit power range of values is 0-7. <br>
...
Description: Verify the network link status 
...
- 2 - Module will automatically execute a one-time Link Check after every payload uplink.
...
| Command      | Input Parameter | Return Value                                  | Return Code |
| ------------ | --------------- | --------------------------------------------- | ----------- |
| `AT+BTIME?`  | -               | `BTIME`: get the Beacon Time (GPS Epoch time) | OK          |
...
This section describes the output from serial/comm lines of the module that can occur at any point in time while the device is operational. The host parser may need to handle this event at any time.
...
| Command      | Input Parameter | Return value                                                                 | Return code |
| ------------ | --------------- | ---------------------------------------------------------------------------- | ----------- |
| `AT+ARSSI?`  | -               | `AT+ARSSI`: view all open channels   RSSI                                    | OK          |
...
| Command           | Input Parameter | Return Value                                                                                       | Return Code                               |
| ----------------- | --------------- | -------------------------------------------------------------------------------------------------- | ----------------------------------------- |
| `AT+MASK? `       | -               | `AT+MASK`: get or set the channel mask to close or open the channel (only for US915 and AU915) | OK                                        |
| `AT+MASK=?`       | -               | `<mask>`                                                                                           | OK                                        |
...
-  In this case, the channel mask mode is only for US915 and AU915.
US915 / AU915 default value is 01FF.
...
| Command           | Input Parameter | Return Value                                                            | Return Code             |
| ----------------- | --------------- | ----------------------------------------------------------------------- | ----------------------- |
| `AT+CHE?`         | -               | `AT+CHE`: get or set eight channels mode; only for US915 and AU915 | OK                      |
| `AT+CHE?=?`       | -               | `<value`>                                                               | OK                      |
...
:::tip 📝 NOTE
- `AT_PARAM_ERROR` is returned when setting wrong or malformed value.
- In this case, the eight-channel mode is only for US915 and AU915. US915 / AU915 range of values is 0~9, e.g. `AT+CHE=1,2,3,4` to enable ch0~31. `<value>`: maximum 12 decimal numbers and the range of values depends on region.
...
| Command          | Input Parameter | Return Value                          | Return Code |
| ---------------- | --------------- | ------------------------------------- | ----------- |
| `AT+CHS?`        | -               | `AT+CHS`: set the single channel mode | OK          |
...
:::tip 📝 NOTE
In this case, the single-channel mode is only for US915 and AU915.
...
| Command           | Input Parameter   | Return Value                            | Return Code                                |
| ----------------- | ----------------- | --------------------------------------- | ------------------------------------------ |
| `AT+BAND?`        | -                 | `AT+BAND`: get or set the active region | OK                                         |
| `AT+BAND=?`       | -                 | 2,3,4,5,6,7,8                       | OK                                         |
| `AT+BAND=<Input>` | 2,3,4,5,6,7,8 | -                                       | OK <br>  AT_PARAM_ERROR <br> AT_BUSY_ERROR |
...
- `AT_PARAM_ERROR` is returned when setting wrong or malformed value.
- `AT_BUSY_ERROR` is returned when the set command process is already running.
- In this case, the default value is 4, and it depends on the region. 2: RU864 3: IN865 4: EU868 5: US915 6: AU915 7: KR920 8: AS923 `<Input>`: 1 decimal integer and the range of values is 0~8.
:::
...
:::tip 📝 NOTE
- `AT_PARAM_ERROR` is returned when setting the wrong or malformed value.
-  In this case, the default value is **7**, and the Spreading Factor range of values is **6~12.** P2P is an SW proprietary protocol, and it depends on HW support in the range of Spreading Factors SF6~SF12.
:::
...
:::tip 📝 NOTE
- `AT_PARAM_ERROR` is returned when setting the wrong or malformed value.
-  In this case, the default value is 125, and the bandwidth only supports 125, 250, and 500.
:::
...
This command provides the configuration of the timeout period for P2P window reception.

| Command            | Input Parameter | Return Value                                                         | Return Code |
| ------------------ | --------------- | -------------------------------------------------------------------- | ----------- |
| `AT+PRECV? `       | -               | `AT+PRECV`: set the timeout for P2P window receive (0-65535&nbsp;ms) | OK          |
...
| Command            | Input Parameter | Return Value                                        | Return Code |
| ------------------ | --------------- | --------------------------------------------------- | ----------- |
| `AT+ENCRY?`        | -               | `AT+ENCRY`: enable the P2P mode with Encryption KEY | OK          |
| `AT+ENCRY=?`       | -               | 0 or 1                                              | OK          |
...
| Command             | Input Parameter | Return Value                                     | Return Code |
| ------------------- | --------------- | ------------------------------------------------ | ----------- |
| `AT+ENCKEY?`        | -               | `AT+ENCRY`: set the P2P mode with Encryption KEY | OK          |
| `AT+ENCKEY=?`       | -               | <8 hex>                                          | OK          |
...
This command provides the configuration of all parameters for the P2P mode.

| Command          | Input Parameter                                                                            | Return Value                      | Return Code            |
| ---------------- | ------------------------------------------------------------------------------------------ | --------------------------------- | ---------------------- |
| `AT+P2P?`        | -                                                                                          | `AT+P2P`: view P2P All Parameters | OK                     |
| `AT+P2P=?`       | -                                                                                          | -                                 | OK <br> AT_ERROR       |
| `AT+P2P=<param>` | `<Frequency>, <Spreading Factor>, <Bandwidth>, <Code Rate>, <Preamble Length>, <TX Power>` | -                                 | OK <br> AT_PARAM_ERROR |

**Example:**
```
AT+P2P=?
Freq = 868000000 Hz, SF = 7,
Bandwidth = 125 KHz, CR = 0, Preamble Length = 8,
Power = 14 dbm
OK
...
AT+P2P=868000000:12:300:3:200:14
...
| Command          | Input Parameter | Return Value                                            | Return Code           |
| ---------------- | --------------- | ------------------------------------------------------- | --------------------- |
| `AT+TTX?`        | -               | `AT+TTX`: set the number of packets sent for RF TX test | OK                    |
...
| Command          | Input Parameter | Return Value                                                | Return Code            |
| ---------------- | --------------- | ----------------------------------------------------------- | ---------------------- |
| `AT+TRX?`        | -               | `AT+TRX`: set the number of packets received for RF RX test | OK                     |
...
Description: Configure the LoRa® RF test 
...
Freq: in Hz, 868000000
Power :[-9 :22]dBm, 14
Bandwidth : Lora [0:7.8125, 1: 15.625, 2: 31.25, 3: 62.5, 4: 125, 5: 250, 6: 500]kHz, 4 or 0 FSK : [4800Hz :467000 Hz]
LoraSf_datarate : Lora[SF5..SF12] FSK [600..300000 bits/s], 12 or 7
CodingRate: Lora Only [1: 4/5, 2: 4/6, 3: 4/7, 4: 4/8], 1
Lna: 0:off 1:On paBoost_state: 0:off 1:On, 0
Modulation: 0: FSK, 1: Lora, 2:BPSK(Tx), 0
PayloadLen: [1:256], 1
PayloadLen: [1:256], 16 or 4
FskdDeviation: FSK only [4800:467000], 25000
Note: no check applied wrt bandwidth. Common practice is to have bandwidth>1,5*fskDev lowDrOpt: Lora Only 0: off, 1:On, 2: Auto (1 when SF11 or SF12, 0 otherwise, 2 or 0
BTproduct: FSK only [0 no Gaussian Filter Applied, 1: BT=0,3, 2: BT=0,5, 3: BT=0,7, 4: BT=1], 3 or 0
```
...
Description: Configure the LoRaWAN® Certification test 
...
:::tip 📝 NOTE
- `AT_BUSY_ERROR` is returned when the start frequency tone process is already running.
- This command is used for the RF certification test and the timer to handler data transmission is equal to 5s.
:::
